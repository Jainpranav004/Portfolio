...
export function ScrambleButton({
  ...
  // For "primary", we invert the visual state:
  // - secondary: transparent -> peach filled on hover
  // - primary: peach filled -> transparent on hover
  const fillVisible = variant === "primary" ? !hovered : hovered;
  ...
  const inner = (
    <>
      <span
        className="absolute inset-0 bg-peach transition-transform duration-500"
        style={{
          transform: fillVisible ? "scaleX(1)" : "scaleX(0)",
          // Keep the "sweep" direction left-to-right for both enter/leave.
          // For primary: contracting uses origin-right, expanding uses origin-left.
          transformOrigin:
            variant === "primary" ? (fillVisible ? "left" : "right") : "left",
      ...
      >
        ...
        {showArrow && (
          <svg
            ...
            style={{
              transform: fillVisible ? "translate(2px, -2px)" : "translate(0, 0)",
              opacity: fillVisible ? 1 : 0.4,
            }}
          >
            <path
